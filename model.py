...
"""
torch.unsqueeze: adds a dimension of size 1 to the specified location, 
            returns a tensor
torch.arange(): returns 1D tensor with values from interval [start,end)
requires_grad(): if u dont want your model to learn the parameters, do false,otherwise true.
nn.Linear(d,d): initializes a learnable matrix of size (d,d)

In the context of 3D tensors, you need to consider the size of the last dimension of the first tensor and the size 
of the second-to-last dimension of the second tensor. If these dimensions match, the tensors can be multiplied.
